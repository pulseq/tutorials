...
%print( '-r150', '-dpng', [out_name '_seq.png']);
...
xlabel('kx / m^-^1'); 
ylabel('kx / m^-^1'); 
print( '-r150', '-dpng', [out_name '_kspace.png']);
...
if nCoils>1
    ...
    sos=sos./max(sos(:));    
    imab(sos);
    imwrite(rot90(sos), [out_name '_img_combined.png']);
else
    imab(abs(images));
    imwrite(rot90(abs(images)./max(abs(images(:)))), [out_name '_img.png']);
end
colormap('gray');
